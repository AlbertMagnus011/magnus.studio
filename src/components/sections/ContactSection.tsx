                ...
                <address className="flex flex-col items-center gap-4">
                    <div>
                        ...
                        <Reveal>
                            ...
                            estarei a disposição para atende-lo <span onClick={copyEmail} className="bg-[#ffe66d] text-[#392a35] px-2 py-1 cursor-pointer" role="button" tabIndex={0}
                            onKeyDown={(e) => {
                                if(e.key === "Enter" || e.key === " ") {
                                    copyEmail();
                                }
                            }}>contato.marcossilva.dev@gmail.com</span>
                            </p>
                        </Reveal>
                    </div>    
                    <ul className="flex gap-5 p-4">
                        {contacts.map((contact, index) => (
                            <Reveal key={contact.id} delay={index * 0.12}>
                                <li className="">
                                    <a href={contact.link} target="_blank" className={`flex flex-col items-center gap-1 p-2 bg-[#ffe66d] border 
                                        rounded-md  ${contact.rotate} shadow-[4px_4px_0px_#ffe66d]/30 transition-transform duration-300 
                                        hover:-translate-y-3 hover:shadow-[6px_6px_0px__#ffe66d]/20  hover:rotate-0`} rel="noopener noreferrer">
